...
import CardChart from "../cardCharts/card-charts.component";
import SpinnerComponent from "../spinner/spinner.component";

import { fetchCourseInformation } from "../../api";
...
const InformationOverview = ({ darkMode, print }) => {
  ...
  Object.keys(data).map((elem) => {
    count += data[elem].count;
    total += data[elem].total;
  });
  courseTotal = {
    count,
    total,
  ...
  courseDaily = {
    count: data["Curso Regular Diurno"].count + data["Curso Diurno"].count,
    total: data["Curso Regular Diurno"].total + data["Curso Diurno"].total,
  };
  courseSesions = {
    count: data["Curso por Encuentros"].count,
    total: data["Curso por Encuentros"].total,
  };
  courseDistances = {
    count: data["Enseñanza a Distancia"].count,
    total: data["Enseñanza a Distancia"].total,
  };
        ...
        <div className={classes.container}>
          ...
          >
            ...
            >
              ...
              <span style={{ margin: "0 0" }}>
                {Math.round((courseSesions.count / courseSesions.total) * 100)}%
              </span>
            </div>
          </div>
        </div>
